...
public class LoginController implements CommunityConstant {
    ...
    public String register(Model model, User user){
        ...
        if(map ==null || map.isEmpty()){
            model.addAttribute("msg","注册成功,我们已经向你的邮箱发送了一封激活邮件，请你尽快激活！");
    ...
    public String login(String username,String password,String code,boolean rememberme,Model model,HttpSession session,HttpServletResponse response){
        ...
        //检查账号密码
        int expiredSecongs =rememberme ? REMEMBER_EXPIRED_SECONDS :DEFAULT_EXPIRED_SECONDS;
        Map<String,Object> map=userService.login(username,password,expiredSecongs);
        if(map.containsKey("ticket")){
            Cookie cookie =new Cookie("ticket",map.get("ticket").toString());
            cookie.setPath("contextPath");
            cookie.setMaxAge(expiredSecongs);
